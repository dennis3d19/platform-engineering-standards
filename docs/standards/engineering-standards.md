...
Treat it as a checklist before opening a pull request, regardless of language or tool.
...
- Automation files (`.pre-commit-config.yaml`, `.github/workflows/`, `.editorconfig`) live at the repository root.
- Configuration that differs per environment (`tst`, `acc`, `prd`) lives in environment-specific directories or value
  files — never in a single file with embedded conditionals.
...
- Resource names follow the pattern `<env>-<app>-<component>` where possible (e.g. `tst-grafana-operator`).
...
- Secret names in Kubernetes follow `<namespace>-<env>-<purpose>` (e.g. `monitoring-tst-grafana-read-token`).
...
- Keep documentation up to date in the same pull request as the code change.
...
- Apply least-privilege: a token scoped to `read_repository` must not carry broader scopes unless explicitly required.
...
- Update `CHANGELOG.md` in the same pull request as the code change.
...
- A `terraform fmt`, `terraform validate`, and `tflint` clean run is required before every pull request.
...
- Pipeline secrets are injected via CI secret variables or Vault; never stored as plain-text in workflow files.
- Pull requests must pass CI before merging to `main`; direct pushes to `main` are blocked.
...
- Every pull request undergoes a security review if it changes: RBAC, network policies, secret handling, authentication flows, or public-facing endpoints.
